	...
	: QMainWindow(parent, flags)
{
	...
	pvlcInstance = libvlc_new(0, NULL);	// Initialize libVLC

	/* Complain in case of broken installation */
	if (pvlcInstance == NULL)
	{
...
int CMPlayer::changeVolume(int vol) { /* Called on volume slider change */
...
void CMPlayer::changePosition(int pos) { /* Called on position slider change */

	if (vlcPlayer)
		libvlc_media_player_set_position(vlcPlayer, (float)pos/1000.0);
}

void CMPlayer::updateInterface() { //Update interface and check if song is finished
